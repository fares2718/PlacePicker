...
export class PlacesService {
  ...
  loadAvailablePlaces() {
    return this.fetchPlaces('http://localhost:5175/api/Places/places');
  }

  loadUserPlaces() {
    return this.fetchPlaces(
      'http://localhost:5175/api/Places/user-places'
    ).pipe(
      tap({
        next: (userPlaces) => {
  ...
  addPlaceToUserPlaces(place: Place) {
    ...
    return this.httpClient
      .put('http://localhost:5175/api/Places/user-places', {
        placeId: place.id,
      })
      .pipe(
        catchError((err) => {
  ...
  removeUserPlace(place: Place) {
    ...
    return this.httpClient
      .delete(`http://localhost:5175/api/Places/user-places/${place.id}`)
      .pipe(
        catchError((err) => {
          this.userPlaces.set(prevPlaces);
